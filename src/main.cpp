...
#if defined(NDEBUG)
# error "FootyCash cannot be compiled without assertions."
#endif
...
int64_t GetProofOfWorkReward(int64_t nFees, int64_t nHeight)
{
    ...
    int64_t nHalf = nHeight / Params().PoWHalfLife();
    nSubsidy >>= nHalf;
...
{
    Bignum nSubsidy, nProp;
    int64_t nHalf = nHeight / Params().PoSHalfLife();

    nProp = Params().StakeCoinYearReward();
    nProp >>= nHalf;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (nStakeReward > nCalculatedStakeReward and nHeight > 259232)
